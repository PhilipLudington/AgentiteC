                                         ...
                                         int spacing, int margin)
{
    ...
    Carbon_Tileset *ts = calloc(1, sizeof(Carbon_Tileset));
    if (!ts) return NULL;
    ...
    ts->sprites = malloc(ts->tile_count * sizeof(Carbon_Sprite));
    if (!ts->sprites) {
        free(ts);
        return NULL;
    }
...
{
    Carbon_TileLayer *layer = calloc(1, sizeof(Carbon_TileLayer));
    ...
    layer->chunks = calloc(total_chunks, sizeof(Carbon_TileChunk *));
    if (!layer->chunks) {
        free(layer->name);
        free(layer);
        return NULL;
...
static Carbon_TileChunk *layer_ensure_chunk(Carbon_TileLayer *layer, int cx, int cy)
{
    ...
    if (!layer->chunks[idx]) {
        layer->chunks[idx] = calloc(1, sizeof(Carbon_TileChunk));
    }
                                      ...
                                      int width, int height)
{
    ...
    Carbon_Tilemap *tm = calloc(1, sizeof(Carbon_Tilemap));
    if (!tm) return NULL;
