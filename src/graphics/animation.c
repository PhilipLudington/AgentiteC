...
Carbon_Animation *carbon_animation_create(Carbon_Sprite *frames, uint32_t frame_count)
{
    ...
    Carbon_Animation *anim = calloc(1, sizeof(Carbon_Animation));
    if (!anim) {
        return NULL;
    }

    anim->frames = malloc(frame_count * sizeof(Carbon_Sprite));
    if (!anim->frames) {
        free(anim);
        return NULL;
    }
                                              ...
                                              int cols, int rows)
{
    ...
    Carbon_Sprite *frames = malloc(frame_count * sizeof(Carbon_Sprite));
    if (!frames) {
        return NULL;
    }
...
void carbon_animation_set_frame_duration(Carbon_Animation *anim, uint32_t frame, float seconds)
{
    ...
    if (!anim->durations) {
        anim->durations = malloc(anim->frame_count * sizeof(float));
        if (!anim->durations) {
            return;
        }
