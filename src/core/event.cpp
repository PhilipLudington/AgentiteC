...
static bool ensure_listener_capacity(Carbon_EventDispatcher *d) {
    ...
    Carbon_Listener *new_listeners = (Carbon_Listener*)realloc(d->listeners,
...
static bool ensure_deferred_capacity(Carbon_EventDispatcher *d) {
    ...
    Carbon_Event *new_queue = (Carbon_Event*)realloc(d->deferred_queue,
...
Carbon_EventDispatcher *carbon_event_dispatcher_create(void) {
    Carbon_EventDispatcher *d = CARBON_ALLOC(Carbon_EventDispatcher);
    if (!d) return NULL;
