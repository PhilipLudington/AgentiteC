...
static void audio_callback(void *userdata, SDL_AudioStream *stream, int additional_amount, int total_amount) {
    ...
    if (samples_needed > audio->mix_buffer_size) {
        audio->mix_buffer = realloc(audio->mix_buffer, samples_needed * sizeof(float));
...
Carbon_Audio *carbon_audio_init(void) {
    Carbon_Audio *audio = calloc(1, sizeof(Carbon_Audio));
    ...
    audio->mix_buffer = malloc(audio->mix_buffer_size * sizeof(float));
...
Carbon_Sound *carbon_sound_load(Carbon_Audio *audio, const char *filepath) {
    ...
    Carbon_Sound *sound = calloc(1, sizeof(Carbon_Sound));
    if (!sound) {
        SDL_free(wav_data);
        return NULL;
    }
...
Carbon_Music *carbon_music_load(Carbon_Audio *audio, const char *filepath) {
    ...
    Carbon_Music *music = calloc(1, sizeof(Carbon_Music));
    if (!music) {
        SDL_free(wav_data);
        return NULL;
    }
