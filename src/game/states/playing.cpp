...
static void playing_enter(Agentite_GameContext *ctx, void *userdata) {
    (void)ctx;
...
static void playing_exit(Agentite_GameContext *ctx, void *userdata) {
    (void)ctx;
    (void)userdata;
...
static void playing_update(Agentite_GameContext *ctx, float dt, void *userdata) {
    PlayingStateData *data = (PlayingStateData *)userdata;
    ...
    /* Update ECS world */
    if (ctx->ecs) {
        agentite_ecs_progress(ctx->ecs, dt);
    }
...
static void playing_render(Agentite_GameContext *ctx,
    ...
    if (ctx->ui) {
        /* Mini HUD in corner */
        if (aui_begin_panel(ctx->ui, "##hud", 10, 10, 150, 50, 0)) {
            aui_label(ctx->ui, "Playing...");
            aui_label(ctx->ui, "ESC to pause");
