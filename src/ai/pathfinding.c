...
{
    heap->entries = malloc(initial_capacity * sizeof(HeapEntry));
...
static bool heap_push(BinaryHeap *heap, int x, int y, float f_cost)
{
    ...
    if (heap->count >= heap->capacity) {
        ...
        HeapEntry *new_entries = realloc(heap->entries, new_cap * sizeof(HeapEntry));
                                      ...
                                      int end_x, int end_y)
{
    ...
    /* Allocate path */
    Carbon_Path *path = malloc(sizeof(Carbon_Path));
    if (!path) return NULL;

    path->points = malloc(length * sizeof(Carbon_PathPoint));
    if (!path->points) {
        free(path);
        return NULL;
    }
...
Carbon_Pathfinder *carbon_pathfinder_create(int width, int height)
{
    ...
    Carbon_Pathfinder *pf = calloc(1, sizeof(Carbon_Pathfinder));
    if (!pf) return NULL;
    ...
    /* Allocate grid */
    pf->grid = malloc(total * sizeof(GridCell));
    if (!pf->grid) {
        free(pf);
        return NULL;
    }
    ...
    /* Allocate node state */
    pf->nodes = malloc(total * sizeof(PathNode));
    if (!pf->nodes) {
        free(pf->grid);
        free(pf);
        return NULL;
                                        ...
                                        const Carbon_PathOptions *options)
{
    ...
    if (start_x == end_x && start_y == end_y) {
        Carbon_Path *path = malloc(sizeof(Carbon_Path));
        if (!path) return NULL;
        path->points = malloc(sizeof(Carbon_PathPoint));
        if (!path->points) {
            free(path);
            return NULL;
        }
...
Carbon_Path *carbon_path_simplify(Carbon_Path *path)
{
    ...
    Carbon_Path *simplified = malloc(sizeof(Carbon_Path));
    if (!simplified) return path;

    simplified->points = malloc(count * sizeof(Carbon_PathPoint));
