 ...
 */
typedef struct {
    char key[AGENTITE_BB_MAX_KEY_LEN];
    Agentite_BBValue value;
    bool used;
} BBEntry;
 ...
 */
typedef struct {
    char key[AGENTITE_BB_MAX_KEY_LEN];  /* Empty = all keys */
    Agentite_BBChangeCallback callback;
    void *userdata;
    uint32_t id;
    bool used;
} BBSubscription;

#define AGENTITE_BB_MAX_SUBSCRIPTIONS 8

/**
 * Blackboard internal structure
 */
struct Agentite_Blackboard {
    /* Key-value storage */
    BBEntry entries[AGENTITE_BB_MAX_ENTRIES];
    int entry_count;

    /* Reservations */
    Agentite_BBReservation reservations[AGENTITE_BB_MAX_RESERVATIONS];
    int reservation_count;

    /* Plans */
    Agentite_BBPlan plans[AGENTITE_BB_MAX_PLANS];
    int plan_count;

    /* History (circular buffer) */
    Agentite_BBHistoryEntry history[AGENTITE_BB_MAX_HISTORY];
    ...
    /* Subscriptions */
    BBSubscription subscriptions[AGENTITE_BB_MAX_SUBSCRIPTIONS];
 ...
 * Find entry by key
 */
static BBEntry *find_entry(Agentite_Blackboard *bb, const char *key) {
    for (int i = 0; i < AGENTITE_BB_MAX_ENTRIES; i++) {
 ...
 */
static BBEntry *get_or_create_entry(Agentite_Blackboard *bb, const char *key) {
    /* Find existing */
    ...
    for (int i = 0; i < AGENTITE_BB_MAX_ENTRIES; i++) {
        if (!bb->entries[i].used) {
            entry = &bb->entries[i];
            entry->used = true;
            strncpy(entry->key, key, AGENTITE_BB_MAX_KEY_LEN - 1);
            entry->key[AGENTITE_BB_MAX_KEY_LEN - 1] = '\0';
            memset(&entry->value, 0, sizeof(Agentite_BBValue));
            bb->entry_count++;
            return entry;
        }
    }
 ...
 */
static void notify_change(Agentite_Blackboard *bb, const char *key,
                          const Agentite_BBValue *old_val,
                          const Agentite_BBValue *new_val) {
    for (int i = 0; i < AGENTITE_BB_MAX_SUBSCRIPTIONS; i++) {
...
Agentite_Blackboard *agentite_blackboard_create(void) {
    Agentite_Blackboard *bb = AGENTITE_ALLOC(Agentite_Blackboard);
    if (!bb) {
        agentite_set_error("agentite_blackboard_create: allocation failed");
        return NULL;
    }
    bb->next_sub_id = 1;
    return bb;
...
void agentite_blackboard_set_int(Agentite_Blackboard *bb, const char *key, int32_t value) {
    ...
    Agentite_BBValue old_val = entry->value;
    entry->value.type = AGENTITE_BB_TYPE_INT;
    entry->value.i32 = value;

    notify_change(bb, key, old_val.type != AGENTITE_BB_TYPE_NONE ? &old_val : NULL,
                  &entry->value);
}
...
void agentite_blackboard_set_int64(Agentite_Blackboard *bb, const char *key, int64_t value) {
    ...
    Agentite_BBValue old_val = entry->value;
    entry->value.type = AGENTITE_BB_TYPE_INT64;
...
void agentite_blackboard_set_float(Agentite_Blackboard *bb, const char *key, float value) {
    ...
    Agentite_BBValue old_val = entry->value;
    entry->value.type = AGENTITE_BB_TYPE_FLOAT;
...
void agentite_blackboard_set_double(Agentite_Blackboard *bb, const char *key, double value) {
    ...
    Agentite_BBValue old_val = entry->value;
    entry->value.type = AGENTITE_BB_TYPE_DOUBLE;
...
void agentite_blackboard_set_bool(Agentite_Blackboard *bb, const char *key, bool value) {
    ...
    Agentite_BBValue old_val = entry->value;
    entry->value.type = AGENTITE_BB_TYPE_BOOL;
    entry->value.b = value;
...
void agentite_blackboard_set_string(Agentite_Blackboard *bb, const char *key, const char *value) {
    ...
    Agentite_BBValue old_val = entry->value;
    entry->value.type = AGENTITE_BB_TYPE_STRING;
    if (value) {
        strncpy(entry->value.str, value, AGENTITE_BB_MAX_STRING_LEN - 1);
        entry->value.str[AGENTITE_BB_MAX_STRING_LEN - 1] = '\0';
    } else {
        entry->value.str[0] = '\0';
    }
...
void agentite_blackboard_set_ptr(Agentite_Blackboard *bb, const char *key, void *value) {
    ...
    Agentite_BBValue old_val = entry->value;
    entry->value.type = AGENTITE_BB_TYPE_PTR;
...
void agentite_blackboard_set_vec2(Agentite_Blackboard *bb, const char *key, float x, float y) {
    ...
    Agentite_BBValue old_val = entry->value;
    entry->value.type = AGENTITE_BB_TYPE_VEC2;
...
void agentite_blackboard_set_vec3(Agentite_Blackboard *bb, const char *key,
    ...
    Agentite_BBValue old_val = entry->value;
    entry->value.type = AGENTITE_BB_TYPE_VEC3;
...
bool agentite_blackboard_has(const Agentite_Blackboard *bb, const char *key) {
    ...
    return find_entry((Agentite_Blackboard *)bb, key) != NULL;
}

Agentite_BBValueType agentite_blackboard_get_type(const Agentite_Blackboard *bb, const char *key) {
    if (!bb || !key) return AGENTITE_BB_TYPE_NONE;

    BBEntry *entry = find_entry((Agentite_Blackboard *)bb, key);
    return entry ? entry->value.type : AGENTITE_BB_TYPE_NONE;
}
...
int32_t agentite_blackboard_get_int(const Agentite_Blackboard *bb, const char *key) {
    ...
    switch (entry->value.type) {
        case AGENTITE_BB_TYPE_INT:
            return entry->value.i32;
        case AGENTITE_BB_TYPE_INT64:
            return (int32_t)entry->value.i64;
        case AGENTITE_BB_TYPE_FLOAT:
            return (int32_t)entry->value.f32;
        case AGENTITE_BB_TYPE_DOUBLE:
            return (int32_t)entry->value.f64;
        case AGENTITE_BB_TYPE_BOOL:
...
bool agentite_blackboard_get_bool(const Agentite_Blackboard *bb, const char *key) {
    ...
    switch (entry->value.type) {
        ...
        case AGENTITE_BB_TYPE_PTR:
            return entry->value.ptr != NULL;
        case AGENTITE_BB_TYPE_STRING:
...
const char *agentite_blackboard_get_string(const Agentite_Blackboard *bb, const char *key) {
    ...
    BBEntry *entry = find_entry((Agentite_Blackboard *)bb, key);
    if (!entry || entry->value.type != AGENTITE_BB_TYPE_STRING) return NULL;
...
void *agentite_blackboard_get_ptr(const Agentite_Blackboard *bb, const char *key) {
    ...
    BBEntry *entry = find_entry((Agentite_Blackboard *)bb, key);
    if (!entry || entry->value.type != AGENTITE_BB_TYPE_PTR) return NULL;
...
bool agentite_blackboard_get_vec2(const Agentite_Blackboard *bb, const char *key,
    ...
    BBEntry *entry = find_entry((Agentite_Blackboard *)bb, key);
    if (!entry || entry->value.type != AGENTITE_BB_TYPE_VEC2) return false;
...
bool agentite_blackboard_get_vec3(const Agentite_Blackboard *bb, const char *key,
    ...
    BBEntry *entry = find_entry((Agentite_Blackboard *)bb, key);
    if (!entry || entry->value.type != AGENTITE_BB_TYPE_VEC3) return false;
...
int32_t agentite_blackboard_inc_int(Agentite_Blackboard *bb, const char *key, int32_t amount) {
    ...
    if (entry->value.type == AGENTITE_BB_TYPE_NONE) {
        entry->value.type = AGENTITE_BB_TYPE_INT;
        entry->value.i32 = 0;
    }

    if (entry->value.type == AGENTITE_BB_TYPE_INT) {
...
int32_t agentite_blackboard_get_int_or(const Agentite_Blackboard *bb, const char *key,
    ...
    return agentite_blackboard_get_int(bb, key);
}
...
float agentite_blackboard_get_float_or(const Agentite_Blackboard *bb, const char *key,
    ...
    return agentite_blackboard_get_float(bb, key);
}
...
bool agentite_blackboard_reserve(Agentite_Blackboard *bb, const char *resource,
    ...
    return agentite_blackboard_reserve_ex(bb, resource, amount, owner, -1);
}
...
bool agentite_blackboard_reserve_ex(Agentite_Blackboard *bb, const char *resource,
    ...
    for (int i = 0; i < bb->reservation_count; i++) {
        Agentite_BBReservation *res = &bb->reservations[i];
    ...
    Agentite_BBReservation *res = &bb->reservations[bb->reservation_count++];
    strncpy(res->resource, resource, AGENTITE_BB_MAX_KEY_LEN - 1);
    res->resource[AGENTITE_BB_MAX_KEY_LEN - 1] = '\0';
    strncpy(res->owner, owner, AGENTITE_BB_MAX_KEY_LEN - 1);
    res->owner[AGENTITE_BB_MAX_KEY_LEN - 1] = '\0';
    res->amount = amount;
    res->turns_remaining = turns;
...
int32_t agentite_blackboard_get_available(const Agentite_Blackboard *bb, const char *resource) {
    ...
    int32_t total = agentite_blackboard_get_int(bb, resource);
    int32_t reserved = agentite_blackboard_get_reserved(bb, resource);
...
int32_t agentite_blackboard_get_reservation(const Agentite_Blackboard *bb, const char *resource,
    ...
    for (int i = 0; i < bb->reservation_count; i++) {
        const Agentite_BBReservation *res = &bb->reservations[i];
        if (strcmp(res->resource, resource) == 0 && strcmp(res->owner, owner) == 0) {
            return res->amount;
        }
    }
...
void agentite_blackboard_publish_plan(Agentite_Blackboard *bb, const char *owner,
    ...
    agentite_blackboard_publish_plan_ex(bb, owner, description, "", -1);
}
...
void agentite_blackboard_publish_plan_ex(Agentite_Blackboard *bb, const char *owner,
        ...
        if (strcmp(bb->plans[i].owner, owner) == 0) {
            ...
            if (description) {
                strncpy(bb->plans[i].description, description,
                        AGENTITE_BB_MAX_STRING_LEN - 1);
                bb->plans[i].description[AGENTITE_BB_MAX_STRING_LEN - 1] = '\0';
            }
            if (target) {
                strncpy(bb->plans[i].target, target, AGENTITE_BB_MAX_KEY_LEN - 1);
                bb->plans[i].target[AGENTITE_BB_MAX_KEY_LEN - 1] = '\0';
            }
    ...
    Agentite_BBPlan *plan = &bb->plans[bb->plan_count++];
    strncpy(plan->owner, owner, AGENTITE_BB_MAX_KEY_LEN - 1);
    plan->owner[AGENTITE_BB_MAX_KEY_LEN - 1] = '\0';

    if (description) {
        strncpy(plan->description, description, AGENTITE_BB_MAX_STRING_LEN - 1);
        plan->description[AGENTITE_BB_MAX_STRING_LEN - 1] = '\0';
    } else {
        plan->description[0] = '\0';
    }

    if (target) {
        strncpy(plan->target, target, AGENTITE_BB_MAX_KEY_LEN - 1);
        plan->target[AGENTITE_BB_MAX_KEY_LEN - 1] = '\0';
    } else {
        plan->target[0] = '\0';
    }
...
void agentite_blackboard_log(Agentite_Blackboard *bb, const char *fmt, ...) {
    ...
    Agentite_BBHistoryEntry *entry = &bb->history[bb->history_head];
    vsnprintf(entry->text, AGENTITE_BB_HISTORY_ENTRY_LEN, fmt, args);
    entry->turn = bb->current_turn;
    entry->timestamp = bb->history_seq++;

    bb->history_head = (bb->history_head + 1) % AGENTITE_BB_MAX_HISTORY;
    if (bb->history_count < AGENTITE_BB_MAX_HISTORY) {
        bb->history_count++;
    }
...
int agentite_blackboard_get_history(const Agentite_Blackboard *bb,
    ...
    for (int i = 0; i < count; i++) {
        int idx = (bb->history_head - 1 - i + AGENTITE_BB_MAX_HISTORY) % AGENTITE_BB_MAX_HISTORY;
...
uint32_t agentite_blackboard_subscribe(Agentite_Blackboard *bb, const char *key,
    ...
    for (int i = 0; i < AGENTITE_BB_MAX_SUBSCRIPTIONS; i++) {
        if (!bb->subscriptions[i].used) {
            ...
            if (key) {
                strncpy(sub->key, key, AGENTITE_BB_MAX_KEY_LEN - 1);
                sub->key[AGENTITE_BB_MAX_KEY_LEN - 1] = '\0';
            } else {
...
void agentite_blackboard_update(Agentite_Blackboard *bb) {
    ...
    for (int i = bb->plan_count - 1; i >= 0; i--) {
        Agentite_BBPlan *plan = &bb->plans[i];
...
int agentite_blackboard_get_keys(const Agentite_Blackboard *bb, const char **out_keys, int max) {
    ...
    int count = 0;
    for (int i = 0; i < AGENTITE_BB_MAX_ENTRIES && count < max; i++) {
        if (bb->entries[i].used) {
...
void agentite_blackboard_copy(Agentite_Blackboard *dest, const Agentite_Blackboard *src) {
    ...
    agentite_blackboard_clear(dest);
