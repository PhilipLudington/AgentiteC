...
Carbon_HTNWorldState *carbon_htn_world_state_create(void) {
    Carbon_HTNWorldState *ws = calloc(1, sizeof(Carbon_HTNWorldState));
    if (!ws) {
...
Carbon_HTNDomain *carbon_htn_domain_create(void) {
    Carbon_HTNDomain *domain = calloc(1, sizeof(Carbon_HTNDomain));
    if (!domain) {
...
Carbon_HTNPlan *carbon_htn_plan(Carbon_HTNDomain *domain,
    ...
    /* Create plan */
    Carbon_HTNPlan *plan = calloc(1, sizeof(Carbon_HTNPlan));
    if (!plan) {
...
Carbon_HTNExecutor *carbon_htn_executor_create(Carbon_HTNDomain *domain) {
    ...
    Carbon_HTNExecutor *exec = calloc(1, sizeof(Carbon_HTNExecutor));
    if (!exec) {
