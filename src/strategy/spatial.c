...
static bool grow_table(Carbon_SpatialIndex *index) {
    ...
    Carbon_SpatialBucket *new_buckets = calloc(new_capacity, sizeof(Carbon_SpatialBucket));
    if (!new_buckets) {
...
Carbon_SpatialIndex *carbon_spatial_create(int capacity) {
    ...
    Carbon_SpatialIndex *index = calloc(1, sizeof(Carbon_SpatialIndex));
    if (!index) {
        carbon_set_error("Spatial: Failed to allocate index");
        return NULL;
    }

    index->buckets = calloc(capacity, sizeof(Carbon_SpatialBucket));
    if (!index->buckets) {
        carbon_set_error("Spatial: Failed to allocate buckets");
        free(index);
        return NULL;
