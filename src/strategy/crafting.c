...
Carbon_RecipeRegistry *carbon_recipe_create(void) {
    Carbon_RecipeRegistry *registry = calloc(1, sizeof(Carbon_RecipeRegistry));
    if (!registry) {
    ...
    registry->recipes = calloc(registry->capacity, sizeof(Carbon_RecipeDef));
    if (!registry->recipes) {
...
Carbon_Crafter *carbon_crafter_create(Carbon_RecipeRegistry *registry) {
    ...
    Carbon_Crafter *crafter = calloc(1, sizeof(Carbon_Crafter));
    if (!crafter) {
