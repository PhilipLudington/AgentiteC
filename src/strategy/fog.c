...
Carbon_FogOfWar *carbon_fog_create(int width, int height) {
    ...
    Carbon_FogOfWar *fog = calloc(1, sizeof(Carbon_FogOfWar));
    if (!fog) {
    ...
    fog->exploration = calloc(grid_size, sizeof(uint8_t));
    fog->visibility = calloc(grid_size, sizeof(uint8_t));
    ...
    fog->sources = calloc(fog->source_capacity, sizeof(Carbon_VisionSourceData));
    if (!fog->sources) {
