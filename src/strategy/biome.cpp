/*
 * Carbon Game Engine - Biome System
 *
 * Terrain types affecting resource distribution and visuals.
 */

#include "agentite/agentite.h"
#include "agentite/biome.h"
#include "agentite/error.h"
#include "agentite/validate.h"

#include <stdlib.h>
#include <string.h>
#include <math.h>

/*============================================================================
 * Internal Structures
 *============================================================================*/

struct Agentite_BiomeSystem {
    Agentite_BiomeDef *biomes;
    int capacity;
    int count;
};

struct Agentite_BiomeMap {
    Agentite_BiomeSystem *system;
    int8_t *data;               /* Biome ID per cell (-1 = invalid) */
    int width;
    int height;
};

/*============================================================================
 * Simple Noise Function
 *============================================================================*/

/* Simple hash-based noise */
static float hash_noise(int x, int y, uint32_t seed) {
    uint32_t n = (uint32_t)x + (uint32_t)y * 57 + seed * 131;
    n = (n << 13) ^ n;
    n = (n * (n * n * 15731 + 789221) + 1376312589);
    return (float)(n & 0x7fffffff) / (float)0x7fffffff;
}

/* Smoothed noise with interpolation */
static float smooth_noise(float x, float y, uint32_t seed) {
    int xi = (int)floorf(x);
    int yi = (int)floorf(y);
    float xf = x - xi;
    float yf = y - yi;

    float n00 = hash_noise(xi, yi, seed);
    float n10 = hash_noise(xi + 1, yi, seed);
    float n01 = hash_noise(xi, yi + 1, seed);
    float n11 = hash_noise(xi + 1, yi + 1, seed);

    /* Smooth interpolation */
    float sx = xf * xf * (3.0f - 2.0f * xf);
    float sy = yf * yf * (3.0f - 2.0f * yf);

    float n0 = n00 * (1.0f - sx) + n10 * sx;
    float n1 = n01 * (1.0f - sx) + n11 * sx;

    return n0 * (1.0f - sy) + n1 * sy;
}

/* Multi-octave noise */
static float fbm_noise(float x, float y, uint32_t seed, int octaves) {
    float value = 0.0f;
    float amplitude = 1.0f;
    float frequency = 0.05f;
    float max_value = 0.0f;

    for (int i = 0; i < octaves; i++) {
        value += smooth_noise(x * frequency, y * frequency, seed + i * 1000) * amplitude;
        max_value += amplitude;
        amplitude *= 0.5f;
        frequency *= 2.0f;
    }

    return value / max_value;
}

/*============================================================================
 * System Creation and Destruction
 *============================================================================*/

Agentite_BiomeSystem *agentite_biome_create(void) {
    Agentite_BiomeSystem *system = AGENTITE_ALLOC(Agentite_BiomeSystem);
    if (!system) {
        agentite_set_error("Failed to allocate biome system");
        return NULL;
    }

    system->capacity = AGENTITE_BIOME_MAX;
    system->biomes = AGENTITE_ALLOC_ARRAY(Agentite_BiomeDef, system->capacity);
    if (!system->biomes) {
        agentite_set_error("Failed to allocate biome storage");
        free(system);
        return NULL;
    }

    return system;
}

void agentite_biome_destroy(Agentite_BiomeSystem *system) {
    if (!system) return;

    free(system->biomes);
    free(system);
}

/*============================================================================
 * Biome Registration
 *============================================================================*/

int agentite_biome_register(Agentite_BiomeSystem *system, const Agentite_BiomeDef *def) {
    AGENTITE_VALIDATE_PTR_RET(system, AGENTITE_BIOME_INVALID);
    AGENTITE_VALIDATE_PTR_RET(def, AGENTITE_BIOME_INVALID);

    if (system->count >= system->capacity) {
        agentite_set_error("Biome system is full");
        return AGENTITE_BIOME_INVALID;
    }

    /* Check for duplicate ID */
    for (int i = 0; i < system->count; i++) {
        if (strcmp(system->biomes[i].id, def->id) == 0) {
            agentite_set_error("Biome with ID '%s' already exists", def->id);
            return AGENTITE_BIOME_INVALID;
        }
    }

    /* Copy biome to system */
    int index = system->count;
    memcpy(&system->biomes[index], def, sizeof(Agentite_BiomeDef));
    system->count++;

    return index;
}

int agentite_biome_count(const Agentite_BiomeSystem *system) {
    AGENTITE_VALIDATE_PTR_RET(system, 0);
    return system->count;
}

const Agentite_BiomeDef *agentite_biome_get(const Agentite_BiomeSystem *system, int biome_id) {
    AGENTITE_VALIDATE_PTR_RET(system, NULL);

    if (biome_id < 0 || biome_id >= system->count) {
        return NULL;
    }

    return &system->biomes[biome_id];
}

const Agentite_BiomeDef *agentite_biome_find(const Agentite_BiomeSystem *system, const char *id) {
    AGENTITE_VALIDATE_PTR_RET(system, NULL);
    AGENTITE_VALIDATE_STRING_RET(id, NULL);

    for (int i = 0; i < system->count; i++) {
        if (strcmp(system->biomes[i].id, id) == 0) {
            return &system->biomes[i];
        }
    }

    return NULL;
}

int agentite_biome_find_index(const Agentite_BiomeSystem *system, const char *id) {
    AGENTITE_VALIDATE_PTR_RET(system, AGENTITE_BIOME_INVALID);
    AGENTITE_VALIDATE_STRING_RET(id, AGENTITE_BIOME_INVALID);

    for (int i = 0; i < system->count; i++) {
        if (strcmp(system->biomes[i].id, id) == 0) {
            return i;
        }
    }

    return AGENTITE_BIOME_INVALID;
}

/*============================================================================
 * Resource Weights
 *============================================================================*/

bool agentite_biome_set_resource_weight(
    Agentite_BiomeSystem *system,
    int biome_id,
    int resource_type,
    float weight)
{
    AGENTITE_VALIDATE_PTR_RET(system, false);

    if (biome_id < 0 || biome_id >= system->count) {
        return false;
    }

    if (resource_type < 0 || resource_type >= AGENTITE_BIOME_MAX_RESOURCES) {
        return false;
    }

    system->biomes[biome_id].resource_weights[resource_type] = weight;
    return true;
}

bool agentite_biome_set_resource_weight_by_id(
    Agentite_BiomeSystem *system,
    const char *id,
    int resource_type,
    float weight)
{
    int biome_id = agentite_biome_find_index(system, id);
    if (biome_id == AGENTITE_BIOME_INVALID) {
        return false;
    }

    return agentite_biome_set_resource_weight(system, biome_id, resource_type, weight);
}

float agentite_biome_get_resource_weight(
    const Agentite_BiomeSystem *system,
    int biome_id,
    int resource_type)
{
    AGENTITE_VALIDATE_PTR_RET(system, 0.0f);

    if (biome_id < 0 || biome_id >= system->count) {
        return 0.0f;
    }

    if (resource_type < 0 || resource_type >= AGENTITE_BIOME_MAX_RESOURCES) {
        return 0.0f;
    }

    return system->biomes[biome_id].resource_weights[resource_type];
}

int agentite_biome_get_best_for_resource(
    const Agentite_BiomeSystem *system,
    int resource_type)
{
    AGENTITE_VALIDATE_PTR_RET(system, AGENTITE_BIOME_INVALID);

    if (resource_type < 0 || resource_type >= AGENTITE_BIOME_MAX_RESOURCES) {
        return AGENTITE_BIOME_INVALID;
    }

    int best_biome = AGENTITE_BIOME_INVALID;
    float best_weight = 0.0f;

    for (int i = 0; i < system->count; i++) {
        float weight = system->biomes[i].resource_weights[resource_type];
        if (weight > best_weight) {
            best_weight = weight;
            best_biome = i;
        }
    }

    return best_biome;
}

int agentite_biome_get_all_for_resource(
    const Agentite_BiomeSystem *system,
    int resource_type,
    int *out_biome_ids,
    int max_count)
{
    AGENTITE_VALIDATE_PTR_RET(system, 0);
    AGENTITE_VALIDATE_PTR_RET(out_biome_ids, 0);

    if (resource_type < 0 || resource_type >= AGENTITE_BIOME_MAX_RESOURCES) {
        return 0;
    }

    int count = 0;
    for (int i = 0; i < system->count && count < max_count; i++) {
        if (system->biomes[i].resource_weights[resource_type] > 0.0f) {
            out_biome_ids[count++] = i;
        }
    }

    return count;
}

/*============================================================================
 * Biome Properties
 *============================================================================*/

const char *agentite_biome_get_name(const Agentite_BiomeSystem *system, int biome_id) {
    const Agentite_BiomeDef *def = agentite_biome_get(system, biome_id);
    return def ? def->name : NULL;
}

uint32_t agentite_biome_get_color(const Agentite_BiomeSystem *system, int biome_id) {
    const Agentite_BiomeDef *def = agentite_biome_get(system, biome_id);
    return def ? def->color : 0;
}

float agentite_biome_get_movement_cost(const Agentite_BiomeSystem *system, int biome_id) {
    const Agentite_BiomeDef *def = agentite_biome_get(system, biome_id);
    return def ? def->movement_cost : 1.0f;
}

float agentite_biome_get_resource_multiplier(const Agentite_BiomeSystem *system, int biome_id) {
    const Agentite_BiomeDef *def = agentite_biome_get(system, biome_id);
    return def ? def->resource_multiplier : 1.0f;
}

float agentite_biome_get_visibility_modifier(const Agentite_BiomeSystem *system, int biome_id) {
    const Agentite_BiomeDef *def = agentite_biome_get(system, biome_id);
    return def ? def->visibility_modifier : 1.0f;
}

float agentite_biome_get_defense_bonus(const Agentite_BiomeSystem *system, int biome_id) {
    const Agentite_BiomeDef *def = agentite_biome_get(system, biome_id);
    return def ? def->defense_bonus : 0.0f;
}

/*============================================================================
 * Biome Flags
 *============================================================================*/

bool agentite_biome_has_flag(const Agentite_BiomeSystem *system, int biome_id, Agentite_BiomeFlags flag) {
    const Agentite_BiomeDef *def = agentite_biome_get(system, biome_id);
    return def ? (def->flags & flag) != 0 : false;
}

bool agentite_biome_is_passable(const Agentite_BiomeSystem *system, int biome_id) {
    return agentite_biome_has_flag(system, biome_id, AGENTITE_BIOME_FLAG_PASSABLE);
}

bool agentite_biome_is_buildable(const Agentite_BiomeSystem *system, int biome_id) {
    return agentite_biome_has_flag(system, biome_id, AGENTITE_BIOME_FLAG_BUILDABLE);
}

bool agentite_biome_is_water(const Agentite_BiomeSystem *system, int biome_id) {
    return agentite_biome_has_flag(system, biome_id, AGENTITE_BIOME_FLAG_WATER);
}

bool agentite_biome_is_hazardous(const Agentite_BiomeSystem *system, int biome_id) {
    return agentite_biome_has_flag(system, biome_id, AGENTITE_BIOME_FLAG_HAZARDOUS);
}

/*============================================================================
 * Biome Map
 *============================================================================*/

Agentite_BiomeMap *agentite_biome_map_create(
    Agentite_BiomeSystem *system,
    int width,
    int height)
{
    AGENTITE_VALIDATE_PTR_RET(system, NULL);

    if (width <= 0 || height <= 0) {
        agentite_set_error("Invalid biome map dimensions");
        return NULL;
    }

    Agentite_BiomeMap *map = AGENTITE_ALLOC(Agentite_BiomeMap);
    if (!map) {
        agentite_set_error("Failed to allocate biome map");
        return NULL;
    }

    map->data = (int8_t*)malloc(width * height);
    if (!map->data) {
        agentite_set_error("Failed to allocate biome map data");
        free(map);
        return NULL;
    }

    /* Initialize all cells to -1 (invalid/unset) */
    memset(map->data, -1, width * height);

    map->system = system;
    map->width = width;
    map->height = height;

    return map;
}

void agentite_biome_map_destroy(Agentite_BiomeMap *map) {
    if (!map) return;

    free(map->data);
    free(map);
}

bool agentite_biome_map_set(Agentite_BiomeMap *map, int x, int y, int biome_id) {
    AGENTITE_VALIDATE_PTR_RET(map, false);

    if (x < 0 || x >= map->width || y < 0 || y >= map->height) {
        return false;
    }

    if (biome_id < -1 || biome_id >= map->system->count) {
        return false;
    }

    map->data[y * map->width + x] = (int8_t)biome_id;
    return true;
}

int agentite_biome_map_get(const Agentite_BiomeMap *map, int x, int y) {
    AGENTITE_VALIDATE_PTR_RET(map, AGENTITE_BIOME_INVALID);

    if (x < 0 || x >= map->width || y < 0 || y >= map->height) {
        return AGENTITE_BIOME_INVALID;
    }

    return map->data[y * map->width + x];
}

void agentite_biome_map_fill_rect(
    Agentite_BiomeMap *map,
    int x, int y,
    int width, int height,
    int biome_id)
{
    AGENTITE_VALIDATE_PTR(map);

    for (int dy = 0; dy < height; dy++) {
        for (int dx = 0; dx < width; dx++) {
            agentite_biome_map_set(map, x + dx, y + dy, biome_id);
        }
    }
}

void agentite_biome_map_fill_circle(
    Agentite_BiomeMap *map,
    int center_x, int center_y,
    int radius,
    int biome_id)
{
    AGENTITE_VALIDATE_PTR(map);

    int r2 = radius * radius;

    for (int dy = -radius; dy <= radius; dy++) {
        for (int dx = -radius; dx <= radius; dx++) {
            if (dx * dx + dy * dy <= r2) {
                agentite_biome_map_set(map, center_x + dx, center_y + dy, biome_id);
            }
        }
    }
}

const Agentite_BiomeDef *agentite_biome_map_get_def(const Agentite_BiomeMap *map, int x, int y) {
    AGENTITE_VALIDATE_PTR_RET(map, NULL);

    int biome_id = agentite_biome_map_get(map, x, y);
    if (biome_id == AGENTITE_BIOME_INVALID) {
        return NULL;
    }

    return agentite_biome_get(map->system, biome_id);
}

float agentite_biome_map_get_movement_cost(const Agentite_BiomeMap *map, int x, int y) {
    const Agentite_BiomeDef *def = agentite_biome_map_get_def(map, x, y);
    return def ? def->movement_cost : 1.0f;
}

float agentite_biome_map_get_resource_weight(
    const Agentite_BiomeMap *map,
    int x, int y,
    int resource_type)
{
    AGENTITE_VALIDATE_PTR_RET(map, 0.0f);

    int biome_id = agentite_biome_map_get(map, x, y);
    if (biome_id == AGENTITE_BIOME_INVALID) {
        return 0.0f;
    }

    return agentite_biome_get_resource_weight(map->system, biome_id, resource_type);
}

bool agentite_biome_map_is_passable(const Agentite_BiomeMap *map, int x, int y) {
    AGENTITE_VALIDATE_PTR_RET(map, false);

    int biome_id = agentite_biome_map_get(map, x, y);
    if (biome_id == AGENTITE_BIOME_INVALID) {
        return false;
    }

    return agentite_biome_is_passable(map->system, biome_id);
}

bool agentite_biome_map_is_buildable(const Agentite_BiomeMap *map, int x, int y) {
    AGENTITE_VALIDATE_PTR_RET(map, false);

    int biome_id = agentite_biome_map_get(map, x, y);
    if (biome_id == AGENTITE_BIOME_INVALID) {
        return false;
    }

    return agentite_biome_is_buildable(map->system, biome_id);
}

void agentite_biome_map_get_size(const Agentite_BiomeMap *map, int *out_width, int *out_height) {
    if (!map) {
        if (out_width) *out_width = 0;
        if (out_height) *out_height = 0;
        return;
    }

    if (out_width) *out_width = map->width;
    if (out_height) *out_height = map->height;
}

int agentite_biome_map_count_biome(const Agentite_BiomeMap *map, int biome_id) {
    AGENTITE_VALIDATE_PTR_RET(map, 0);

    int count = 0;
    int size = map->width * map->height;

    for (int i = 0; i < size; i++) {
        if (map->data[i] == biome_id) {
            count++;
        }
    }

    return count;
}

void agentite_biome_map_get_stats(const Agentite_BiomeMap *map, int *out_counts) {
    AGENTITE_VALIDATE_PTR(map);
    AGENTITE_VALIDATE_PTR(out_counts);

    /* Initialize counts to 0 */
    memset(out_counts, 0, AGENTITE_BIOME_MAX * sizeof(int));

    int size = map->width * map->height;
    for (int i = 0; i < size; i++) {
        int8_t biome_id = map->data[i];
        if (biome_id >= 0 && biome_id < AGENTITE_BIOME_MAX) {
            out_counts[biome_id]++;
        }
    }
}

/*============================================================================
 * Generation Helpers
 *============================================================================*/

void agentite_biome_map_generate_noise(
    Agentite_BiomeMap *map,
    const int *biome_ids,
    const float *thresholds,
    int count,
    uint32_t seed)
{
    AGENTITE_VALIDATE_PTR(map);
    AGENTITE_VALIDATE_PTR(biome_ids);
    AGENTITE_VALIDATE_PTR(thresholds);

    if (count <= 0) return;

    for (int y = 0; y < map->height; y++) {
        for (int x = 0; x < map->width; x++) {
            float noise = fbm_noise((float)x, (float)y, seed, 4);

            /* Find biome based on threshold */
            int biome_id = biome_ids[0];
            for (int i = 0; i < count; i++) {
                if (noise >= thresholds[i]) {
                    biome_id = biome_ids[i];
                }
            }

            agentite_biome_map_set(map, x, y, biome_id);
        }
    }
}

void agentite_biome_map_smooth(Agentite_BiomeMap *map, int passes) {
    AGENTITE_VALIDATE_PTR(map);

    if (passes <= 0) return;

    int8_t *temp = (int8_t*)malloc(map->width * map->height);
    if (!temp) return;

    for (int pass = 0; pass < passes; pass++) {
        for (int y = 0; y < map->height; y++) {
            for (int x = 0; x < map->width; x++) {
                int8_t current = map->data[y * map->width + x];

                /* Count neighbors of same type */
                int same_count = 0;
                int different_biome = -1;
                int different_count = 0;

                for (int dy = -1; dy <= 1; dy++) {
                    for (int dx = -1; dx <= 1; dx++) {
                        if (dx == 0 && dy == 0) continue;

                        int nx = x + dx;
                        int ny = y + dy;

                        if (nx >= 0 && nx < map->width && ny >= 0 && ny < map->height) {
                            int8_t neighbor = map->data[ny * map->width + nx];
                            if (neighbor == current) {
                                same_count++;
                            } else if (neighbor >= 0) {
                                if (different_biome < 0 || neighbor == different_biome) {
                                    different_biome = neighbor;
                                    different_count++;
                                }
                            }
                        }
                    }
                }

                /* If more neighbors are different, change to that biome */
                if (different_count > same_count && different_biome >= 0) {
                    temp[y * map->width + x] = (int8_t)different_biome;
                } else {
                    temp[y * map->width + x] = current;
                }
            }
        }

        /* Copy back */
        memcpy(map->data, temp, map->width * map->height);
    }

    free(temp);
}

/*============================================================================
 * Utility Functions
 *============================================================================*/

Agentite_BiomeDef agentite_biome_default_def(void) {
    Agentite_BiomeDef def = {};

    strncpy(def.id, "default", sizeof(def.id) - 1);
    def.id[sizeof(def.id) - 1] = '\0';
    strncpy(def.name, "Default", sizeof(def.name) - 1);
    def.name[sizeof(def.name) - 1] = '\0';
    def.color = 0xFF808080;  /* Gray */
    def.movement_cost = 1.0f;
    def.resource_multiplier = 1.0f;
    def.visibility_modifier = 1.0f;
    def.defense_bonus = 0.0f;
    def.flags = AGENTITE_BIOME_FLAG_PASSABLE | AGENTITE_BIOME_FLAG_BUILDABLE;

    return def;
}

uint32_t agentite_biome_rgb(uint8_t r, uint8_t g, uint8_t b) {
    return 0xFF000000 | ((uint32_t)b << 16) | ((uint32_t)g << 8) | (uint32_t)r;
}

uint32_t agentite_biome_rgba(uint8_t r, uint8_t g, uint8_t b, uint8_t a) {
    return ((uint32_t)a << 24) | ((uint32_t)b << 16) | ((uint32_t)g << 8) | (uint32_t)r;
}
