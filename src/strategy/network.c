    ...
    for (int i = 0; i < network->capacity; i++) {
        ...
        if (group_idx < 0) {
            ...
            if (network->group_count >= network->group_capacity) {
                ...
                Carbon_NetworkGroup *new_groups = realloc(network->groups,
...
Carbon_NetworkSystem *carbon_network_create(void) {
    Carbon_NetworkSystem *network = calloc(1, sizeof(Carbon_NetworkSystem));
    if (!network) {
    ...
    network->nodes = calloc(initial_capacity, sizeof(NetworkNodeInternal));
    if (!network->nodes) {
...
uint32_t carbon_network_add_node(Carbon_NetworkSystem *network, int x, int y, int radius) {
    ...
    if (network->count >= network->capacity) {
        ...
        NetworkNodeInternal *new_nodes = realloc(network->nodes,
...
void carbon_network_recalculate(Carbon_NetworkSystem *network) {
    ...
    if (network->callback) {
        old_groups = malloc(network->capacity * sizeof(uint32_t));
        if (old_groups) {
            for (int i = 0; i < network->capacity; i++) {
                if (network->nodes[i].in_use) {
