...
Carbon_RateTracker *carbon_rate_create(int metric_count, float sample_interval, int history_size) {
    ...
    Carbon_RateTracker *tracker = calloc(1, sizeof(Carbon_RateTracker));
    if (!tracker) {
        carbon_set_error("Rate: Failed to allocate tracker");
        return NULL;
    }

    tracker->metrics = calloc(metric_count, sizeof(Carbon_MetricTracker));
    if (!tracker->metrics) {
    ...
    for (int i = 0; i < metric_count; i++) {
        tracker->metrics[i].samples = calloc(history_size, sizeof(Carbon_RateSample));
