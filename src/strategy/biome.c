...
Carbon_BiomeSystem *carbon_biome_create(void) {
    Carbon_BiomeSystem *system = calloc(1, sizeof(Carbon_BiomeSystem));
    if (!system) {
    ...
    system->biomes = calloc(system->capacity, sizeof(Carbon_BiomeDef));
    if (!system->biomes) {
    ...
    int height)
{
    ...
    Carbon_BiomeMap *map = calloc(1, sizeof(Carbon_BiomeMap));
    if (!map) {
...
void carbon_biome_map_smooth(Carbon_BiomeMap *map, int passes) {
    ...
    int8_t *temp = malloc(map->width * map->height);
    if (!temp) return;
