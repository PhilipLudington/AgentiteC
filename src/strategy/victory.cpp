 ...
 */
struct Agentite_VictoryManager {
    /* Registered conditions */
    Agentite_VictoryCondition conditions[AGENTITE_VICTORY_MAX_CONDITIONS];
    int condition_count;

    /* Type to index mapping (-1 = not registered) */
    int type_to_index[AGENTITE_VICTORY_MAX_CONDITIONS + AGENTITE_VICTORY_USER];

    /* Per-faction progress */
    Agentite_VictoryProgress factions[AGENTITE_VICTORY_MAX_FACTIONS];
    bool faction_active[AGENTITE_VICTORY_MAX_FACTIONS];
    int faction_count;

    /* Current game state */
    Agentite_VictoryState state;
    uint32_t current_turn;

    /* Custom checkers */
    VictoryChecker checkers[AGENTITE_VICTORY_MAX_CONDITIONS];

    /* Callback */
    Agentite_VictoryCallback on_victory;
 ...
 */
static int find_condition_by_type(const Agentite_VictoryManager *vm, int type) {
    if (!vm) return -1;

    /* Check type_to_index mapping for fast lookup */
    if (type >= 0 && type < AGENTITE_VICTORY_MAX_CONDITIONS + AGENTITE_VICTORY_USER) {
 ...
 */
static void emit_progress_event(Agentite_VictoryManager *vm, int type, float progress) {
    if (!vm || !vm->events) return;
    agentite_event_emit_victory_progress(vm->events, type, progress);
}

/**
 * Emit victory achieved event
 */
static void emit_victory_event(Agentite_VictoryManager *vm, int type, int winner_id) {
    if (!vm || !vm->events) return;
    agentite_event_emit_victory(vm->events, type, winner_id);
}
...
Agentite_VictoryManager *agentite_victory_create(void) {
    return agentite_victory_create_with_events(NULL);
}

Agentite_VictoryManager *agentite_victory_create_with_events(Agentite_EventDispatcher *events) {
    Agentite_VictoryManager *vm = AGENTITE_ALLOC(Agentite_VictoryManager);
    if (!vm) {
        agentite_set_error("agentite_victory_create: allocation failed");
        return NULL;
    }

    vm->events = events;
    vm->state.winner_id = -1;
    vm->state.victory_type = AGENTITE_VICTORY_NONE;

    /* Initialize type mapping to -1 (not registered) */
    for (int i = 0; i < AGENTITE_VICTORY_MAX_CONDITIONS + AGENTITE_VICTORY_USER; i++) {
...
int agentite_victory_register(Agentite_VictoryManager *vm,
    ...
    if (!vm || !cond) {
        agentite_set_error("agentite_victory_register: null parameter");
        return -1;
    }

    if (vm->condition_count >= AGENTITE_VICTORY_MAX_CONDITIONS) {
        agentite_set_error("agentite_victory_register: max conditions reached");
        return -1;
    }

    /* Check for duplicate type */
    if (find_condition_by_type(vm, cond->type) >= 0) {
        agentite_set_error("agentite_victory_register: type %d already registered", cond->type);
        return -1;
    }
    ...
    /* Update type mapping */
    if (cond->type >= 0 && cond->type < AGENTITE_VICTORY_MAX_CONDITIONS + AGENTITE_VICTORY_USER) {
...
void agentite_victory_init_faction(Agentite_VictoryManager *vm, int faction_id) {
    if (!vm || faction_id < 0 || faction_id >= AGENTITE_VICTORY_MAX_FACTIONS) {
        return;
    }

    memset(&vm->factions[faction_id], 0, sizeof(Agentite_VictoryProgress));
...
void agentite_victory_update_score(Agentite_VictoryManager *vm,
    ...
    const Agentite_VictoryCondition *cond = &vm->conditions[index];
...
void agentite_victory_add_score(Agentite_VictoryManager *vm,
    ...
    agentite_victory_update_score(vm, faction_id, type, new_score);
}
...
int agentite_victory_active_faction_count(const Agentite_VictoryManager *vm) {
    ...
    int count = 0;
    for (int i = 0; i < AGENTITE_VICTORY_MAX_FACTIONS; i++) {
...
bool agentite_victory_check(Agentite_VictoryManager *vm) {
    ...
    for (int c = 0; c < vm->condition_count; c++) {
        const Agentite_VictoryCondition *cond = &vm->conditions[c];
        if (!cond->enabled) continue;

        /* If there's a custom checker, use it */
        if (vm->checkers[c].checker) {
            for (int f = 0; f < AGENTITE_VICTORY_MAX_FACTIONS; f++) {
                ...
                if (vm->checkers[c].checker(f, cond->type, &progress,
                    ...
                    agentite_victory_declare(vm, f, cond->type, NULL);
                    return true;
                }
    ...
    int index = find_condition_by_type(vm, AGENTITE_VICTORY_ELIMINATION);
    if (index >= 0 && vm->conditions[index].enabled) {
        ...
        if (survivor_count == 1 && survivor >= 0) {
            agentite_victory_declare(vm, survivor, AGENTITE_VICTORY_ELIMINATION,
    ...
    index = find_condition_by_type(vm, AGENTITE_VICTORY_SCORE);
    if (index >= 0 && vm->conditions[index].enabled) {
        const Agentite_VictoryCondition *cond = &vm->conditions[index];
        if (cond->target_turn > 0 && vm->current_turn >= (uint32_t)cond->target_turn) {
            /* Time's up - highest score wins */
            int leader = agentite_victory_get_score_leader(vm);
            if (leader >= 0) {
                agentite_victory_declare(vm, leader, AGENTITE_VICTORY_SCORE,
...
void agentite_victory_declare(Agentite_VictoryManager *vm,
    ...
    vm->state.winning_score = agentite_victory_calculate_score(vm, faction_id);
    ...
    if (message) {
        ...
        const Agentite_VictoryCondition *cond = agentite_victory_get_by_type(vm, type);
        if (cond) {
...
int agentite_victory_get_winning_type(const Agentite_VictoryManager *vm) {
    return vm ? vm->state.victory_type : AGENTITE_VICTORY_NONE;
}
...
void agentite_victory_reset(Agentite_VictoryManager *vm) {
    ...
    /* Reset victory state */
    memset(&vm->state, 0, sizeof(Agentite_VictoryState));
    vm->state.winner_id = -1;
    vm->state.victory_type = AGENTITE_VICTORY_NONE;

    /* Reset faction progress */
    for (int i = 0; i < AGENTITE_VICTORY_MAX_FACTIONS; i++) {
        memset(&vm->factions[i], 0, sizeof(Agentite_VictoryProgress));
...
int32_t agentite_victory_calculate_score(const Agentite_VictoryManager *vm,
    ...
    int32_t total = 0;
    const Agentite_VictoryProgress *progress = &vm->factions[faction_id];

    for (int i = 0; i < vm->condition_count; i++) {
        const Agentite_VictoryCondition *cond = &vm->conditions[i];
...
int agentite_victory_get_score_leader(const Agentite_VictoryManager *vm) {
    ...
    for (int f = 0; f < AGENTITE_VICTORY_MAX_FACTIONS; f++) {
        ...
        int32_t score = agentite_victory_calculate_score(vm, f);
        if (score > highest) {
            highest = score;
            leader = f;
        }
...
const char *agentite_victory_type_name(int type) {
    switch (type) {
        case AGENTITE_VICTORY_NONE:        return "None";
        case AGENTITE_VICTORY_DOMINATION:  return "Domination";
        case AGENTITE_VICTORY_ELIMINATION: return "Elimination";
        case AGENTITE_VICTORY_TECHNOLOGY:  return "Technology";
        case AGENTITE_VICTORY_ECONOMIC:    return "Economic";
        case AGENTITE_VICTORY_SCORE:       return "Score";
        case AGENTITE_VICTORY_TIME:        return "Time";
        case AGENTITE_VICTORY_OBJECTIVE:   return "Objective";
        case AGENTITE_VICTORY_WONDER:      return "Wonder";
        case AGENTITE_VICTORY_DIPLOMATIC:  return "Diplomatic";
        case AGENTITE_VICTORY_CULTURAL:    return "Cultural";
        default:
            if (type >= AGENTITE_VICTORY_USER) {
                return "Custom";
            }
            return "Unknown";
    }
...
char *agentite_victory_format_progress(const Agentite_VictoryManager *vm,
    ...
    float progress = agentite_victory_get_progress(vm, faction_id, type);
    int index = find_condition_by_type(vm, type);
