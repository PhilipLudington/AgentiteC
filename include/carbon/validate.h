#ifndef CARBON_VALIDATE_H
#define CARBON_VALIDATE_H
 ...
 *       CARBON_VALIDATE_RANGE_RET(count, 1, 100, false);
 *       // ... actual implementation ...
 *       return true;
 *   }
 *
 *   void do_something_void(void *ptr) {
 *       CARBON_VALIDATE_PTR(ptr);
 *       // ... implementation ...
 *   }
 */
 ...
 * More efficient than multiple CARBON_VALIDATE_PTR calls.
 */
#define CARBON_VALIDATE_PTRS2(p1, p2) \
    do { \
        if (!(p1)) { carbon_set_error("%s: null pointer: %s", __func__, #p1); return; } \
        if (!(p2)) { carbon_set_error("%s: null pointer: %s", __func__, #p2); return; } \
    } while(0)

#define CARBON_VALIDATE_PTRS3(p1, p2, p3) \
    do { \
        if (!(p1)) { carbon_set_error("%s: null pointer: %s", __func__, #p1); return; } \
        if (!(p2)) { carbon_set_error("%s: null pointer: %s", __func__, #p2); return; } \
        if (!(p3)) { carbon_set_error("%s: null pointer: %s", __func__, #p3); return; } \
    } while(0)

/**
 * Validate multiple pointers (with return value).
 */
#define CARBON_VALIDATE_PTRS2_RET(p1, p2, ret) \
    do { \
        if (!(p1)) { carbon_set_error("%s: null pointer: %s", __func__, #p1); return (ret); } \
        if (!(p2)) { carbon_set_error("%s: null pointer: %s", __func__, #p2); return (ret); } \
    } while(0)

#define CARBON_VALIDATE_PTRS3_RET(p1, p2, p3, ret) \
    do { \
        if (!(p1)) { carbon_set_error("%s: null pointer: %s", __func__, #p1); return (ret); } \
        if (!(p2)) { carbon_set_error("%s: null pointer: %s", __func__, #p2); return (ret); } \
        if (!(p3)) { carbon_set_error("%s: null pointer: %s", __func__, #p3); return (ret); } \
    } while(0)
 ...
 */
#define CARBON_VALIDATE_INDEX(index, count) \
    do { \
        if ((size_t)(index) >= (size_t)(count)) { \
            carbon_set_error("%s: index out of bounds: %s (%zu >= %zu)", \
 ...
 */
#define CARBON_VALIDATE_INDEX_RET(index, count, ret) \
    do { \
        if ((size_t)(index) >= (size_t)(count)) { \
            carbon_set_error("%s: index out of bounds: %s (%zu >= %zu)", \
                            __func__, #index, (size_t)(index), (size_t)(count)); \
 ...
 */
#define CARBON_VALIDATE_RANGE_F(val, min, max) \
    do { \
        if ((val) < (min) || (val) > (max)) { \
            carbon_set_error("%s: %s out of range [%.2f, %.2f]: %.2f", \
 ...
 */
#define CARBON_VALIDATE_RANGE_F_RET(val, min, max, ret) \
    do { \
        if ((val) < (min) || (val) > (max)) { \
            carbon_set_error("%s: %s out of range [%.2f, %.2f]: %.2f", \
                            __func__, #val, (float)(min), (float)(max), (float)(val)); \
 ...
 */
#define CARBON_VALIDATE_POSITIVE(val) \
    do { \
        if ((val) <= 0) { \
            carbon_set_error("%s: %s must be positive: %d", __func__, #val, (int)(val)); \
            return; \
        } \
    } while(0)

#define CARBON_VALIDATE_POSITIVE_RET(val, ret) \
    do { \
        if ((val) <= 0) { \
            carbon_set_error("%s: %s must be positive: %d", __func__, #val, (int)(val)); \
            return (ret); \
 ...
 */
#define CARBON_VALIDATE_NON_NEGATIVE(val) \
    do { \
        if ((val) < 0) { \
            carbon_set_error("%s: %s must be non-negative: %d", __func__, #val, (int)(val)); \
            return; \
        } \
    } while(0)

#define CARBON_VALIDATE_NON_NEGATIVE_RET(val, ret) \
    do { \
        if ((val) < 0) { \
            carbon_set_error("%s: %s must be non-negative: %d", __func__, #val, (int)(val)); \
            return (ret); \
 ...
 */
#define CARBON_VALIDATE_STRING(str) \
    do { \
        if (!(str) || (str)[0] == '\0') { \
            carbon_set_error("%s: null or empty string: %s", __func__, #str); \
            return; \
        } \
    } while(0)
 ...
 */
#define CARBON_VALIDATE_STRING_RET(str, ret) \
    do { \
        if (!(str) || (str)[0] == '\0') { \
            carbon_set_error("%s: null or empty string: %s", __func__, #str); \
            return (ret); \
 ...
 */
#define CARBON_VALIDATE_COND(cond, msg) \
    do { \
        if (!(cond)) { \
            carbon_set_error("%s: %s", __func__, (msg)); \
            return; \
        } \
    } while(0)
 ...
 */
#define CARBON_VALIDATE_COND_RET(cond, msg, ret) \
    do { \
        if (!(cond)) { \
            carbon_set_error("%s: %s", __func__, (msg)); \
            return (ret); \
 ...
 * Note: For full validation with ecs_is_alive(), use CARBON_VALIDATE_ENTITY_ALIVE.
 */
#define CARBON_VALIDATE_ENTITY(entity) \
    do { \
        if ((entity) == 0) { \
...
#define CARBON_VALIDATE_ENTITY_RET(entity, ret) \
    do { \
        if ((entity) == 0) { \
            carbon_set_error("%s: invalid entity: %s", __func__, #entity); \
            return (ret); \
 ...
 */
#define CARBON_VALIDATE_ENTITY_ALIVE(world, entity) \
    do { \
        if ((entity) == 0 || !ecs_is_alive((world), (entity))) { \
            carbon_set_error("%s: entity not alive: %s", __func__, #entity); \
            return; \
        } \
    } while(0)

#define CARBON_VALIDATE_ENTITY_ALIVE_RET(world, entity, ret) \
    do { \
        if ((entity) == 0 || !ecs_is_alive((world), (entity))) { \
            carbon_set_error("%s: entity not alive: %s", __func__, #entity); \
            return (ret); \
...
#define CARBON_UNREACHABLE() ((void)0)
#endif
 ...
 */
#define CARBON_WARN_IF_NULL(ptr) \
    do { \
        if (!(ptr)) { \
...
#define CARBON_WARN_IF(cond, msg) \
    do { \
        if (cond) { \
 ...
 */
#define CARBON_INVALID_ID    0
#define CARBON_INVALID_INDEX ((size_t)-1)

/**
 * Quick check and return NULL pattern.
 */
#define CARBON_RETURN_NULL_IF(cond) \
    do { if (cond) return NULL; } while(0)

/**
 * Quick check and return false pattern.
 */
#define CARBON_RETURN_FALSE_IF(cond) \
    do { if (cond) return false; } while(0)

/**
 * Quick check and return pattern.
 */
#define CARBON_RETURN_IF(cond) \
    do { if (cond) return; } while(0)

#define CARBON_RETURN_VAL_IF(cond, val) \
    do { if (cond) return (val); } while(0)

#endif /* CARBON_VALIDATE_H */
