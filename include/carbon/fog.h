...
#ifndef CARBON_FOG_MAX_SOURCES
#define CARBON_FOG_MAX_SOURCES 256
#endif

/**
 * @brief Visibility state for a cell
 */
typedef enum Carbon_VisibilityState {
    CARBON_VIS_UNEXPLORED = 0,  /**< Never seen - completely black */
    CARBON_VIS_EXPLORED   = 1,  /**< Previously seen - shroud/fog */
    CARBON_VIS_VISIBLE    = 2   /**< Currently visible - full clarity */
} Carbon_VisibilityState;

/**
 * @brief Opaque fog of war handle
 */
typedef struct Carbon_FogOfWar Carbon_FogOfWar;

/**
 * @brief Vision source handle
 */
typedef uint32_t Carbon_VisionSource;

/**
 * @brief Invalid vision source constant
 */
#define CARBON_VISION_SOURCE_INVALID 0
 ...
 */
typedef void (*Carbon_ExplorationCallback)(Carbon_FogOfWar *fog, int x, int y, void *userdata);
 ...
 * @return Vision source handle or CARBON_VISION_SOURCE_INVALID on failure
 */
 ...
 */
typedef bool (*Carbon_VisionBlockerCallback)(int x, int y, void *userdata);
