#ifndef AGENTITE_CONTAINERS_H
#define AGENTITE_CONTAINERS_H
 ...
 *   agentite_array_push(&numbers, 42);
 *   agentite_array_push(&numbers, 17);
 *   int val = numbers.data[0];  // 42
 *   agentite_array_free(&numbers);
 *
 *   // Random selection
 *   int items[] = {1, 2, 3, 4, 5};
 *   int chosen = agentite_random_choice(items, 5);
 *
 *   // Shuffle
 *   agentite_shuffle_array(items, 5);
 */
 ...
 *
 * Example:
 *   Agentite_Array(float) values;
 *   Agentite_Array(struct Entity) entities;
 */
#define Agentite_Array(T) struct { T *data; size_t count; size_t capacity; }

/**
 * Initialize a dynamic array to empty state.
 */
#define agentite_array_init(arr) \
    do { \
        (arr)->data = NULL; \
        (arr)->count = 0; \
        (arr)->capacity = 0; \
 ...
 */
#define agentite_array_free(arr) \
    do { \
        free((arr)->data); \
        (arr)->data = NULL; \
        (arr)->count = 0; \
 ...
 */
#define agentite_array_push(arr, item) \
    do { \
 ...
 */
#define agentite_array_remove(arr, index) \
    do { \
 ...
 */
#define agentite_array_remove_swap(arr, index) \
    do { \
        if ((size_t)(index) < (arr)->count) { \
            if ((size_t)(index) < (arr)->count - 1) { \
 ...
 */
#define agentite_array_reserve(arr, cap) \
    do { \
        if ((cap) > (arr)->capacity) { \
 ...
 */
#define agentite_array_resize(arr, new_count) \
    do { \
        if ((new_count) > (arr)->capacity) { \
            agentite_array_reserve((arr), (new_count)); \
        } \
 ...
 */
#define agentite_array_shrink(arr) \
    do { \
 ...
 */
#define agentite_array_insert(arr, index, item) \
    do { \
        if ((arr)->count >= (arr)->capacity) { \
            size_t _new_cap = (arr)->capacity == 0 ? 8 : (arr)->capacity * 2; \
            void *_new_data = realloc((arr)->data, _new_cap * sizeof(*(arr)->data)); \
 ...
 */
#define agentite_array_contains(arr, item) \
    ({ \
        bool _found = false; \
 ...
 */
#define agentite_array_find(arr, item) \
    ({ \
 ...
 *
 * Example:
 *   agentite_array_foreach(arr, int, item) {
 *       printf("%d\n", item);
 *   }
 */
#define agentite_array_foreach(arr, type, var) \
 ...
 *
 * Example:
 *   agentite_array_foreach_i(arr, i, int, item) {
 *       printf("[%zu] = %d\n", i, item);
 *   }
 */
#define agentite_array_foreach_i(arr, idx_var, type, var) \
 ...
 * Example:
 *   int items[] = {10, 20, 30};
 *   int chosen = agentite_random_choice(items, 3);  // Returns 10, 20, or 30
 */
#define agentite_random_choice(arr, count) \
    ((count) > 0 ? (arr)[agentite_rand_index(count)] : (arr)[0])

/**
 * Random choice from Agentite_Array.
 */
#define agentite_array_random_choice(arr) \
    ((arr)->count > 0 ? (arr)->data[agentite_rand_index((arr)->count)] : (arr)->data[0])
 ...
 *   agentite_shuffle_array(items, 5);
 */
#define agentite_shuffle_array(arr, count) \
    agentite_shuffle((arr), (count), sizeof(*(arr)))

/**
 * Shuffle a Agentite_Array in place.
 */
#define agentite_array_shuffle(arr) \
    agentite_shuffle((arr)->data, (arr)->count, sizeof(*(arr)->data))
 ...
 */
#define agentite_array_min_index(arr) \
    ({ \
        size_t _min_idx = 0; \
        for (size_t _i = 1; _i < (arr)->count; _i++) { \
 ...
 */
#define agentite_array_max_index(arr) \
    ({ \
        size_t _max_idx = 0; \
        for (size_t _i = 1; _i < (arr)->count; _i++) { \
 ...
 * Returns 0 for empty arrays.
 */
#define agentite_array_avg(arr) \
    ((arr)->count > 0 ? (double)agentite_array_sum(arr) / (double)(arr)->count : 0.0)
 ...
 */
#define Agentite_Stack(T) Agentite_Array(T)

#define agentite_stack_init(s)     agentite_array_init(s)
#define agentite_stack_free(s)     agentite_array_free(s)
#define agentite_stack_push(s, v)  agentite_array_push(s, v)
#define agentite_stack_pop(s)      agentite_array_pop(s)
#define agentite_stack_peek(s)     agentite_array_last(s)
#define agentite_stack_empty(s)    ((s)->count == 0)
#define agentite_stack_size(s)     ((s)->count)
 ...
 */
#define Agentite_RingBuffer(T, N) struct { T data[N]; size_t head; size_t tail; size_t count; }

#define agentite_ring_init(rb) \
    do { (rb)->head = 0; (rb)->tail = 0; (rb)->count = 0; } while(0)

#define agentite_ring_capacity(rb) \
    (sizeof((rb)->data) / sizeof((rb)->data[0]))

#define agentite_ring_full(rb) \
    ((rb)->count >= agentite_ring_capacity(rb))

#define agentite_ring_empty(rb) \
    ((rb)->count == 0)

#define agentite_ring_push(rb, item) \
    do { \
        if (!agentite_ring_full(rb)) { \
            (rb)->data[(rb)->tail] = (item); \
            (rb)->tail = ((rb)->tail + 1) % agentite_ring_capacity(rb); \
            (rb)->count++; \
        } \
    } while(0)

#define agentite_ring_pop(rb) \
    ({ \
        __typeof__((rb)->data[0]) _val = (rb)->data[(rb)->head]; \
        if ((rb)->count > 0) { \
            (rb)->head = ((rb)->head + 1) % agentite_ring_capacity(rb); \
            (rb)->count--; \
        } \
        _val; \
    })

#define agentite_ring_peek(rb) \
    ((rb)->data[(rb)->head])

#endif /* AGENTITE_CONTAINERS_H */
